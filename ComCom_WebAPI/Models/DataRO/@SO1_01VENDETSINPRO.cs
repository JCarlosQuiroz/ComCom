using System;
using System.Collections.Generic;

namespace ComCom_WebAPI.DataRO
{
    public partial class @SO1_01VENDETSINPRO
    {
        public string? Code { get; set; }
        public string? Name { get; set; }
        public string U_SO1_PRECIOMANUAL { get; set; } = null!;
        public string? U_SO1_MONEDA { get; set; }
        public decimal? U_SO1_TIPOCAMBIO { get; set; }
        public string? U_SO1_FOLIO { get; set; }
        public string? U_SO1_NUMEROARTICULO { get; set; }
        public short? U_SO1_NUMPARTIDA { get; set; }
        public decimal? U_SO1_CANTIDAD { get; set; }
        public decimal? U_SO1_PBSD { get; set; }
        public decimal? U_SO1_PBCD { get; set; }
        public decimal? U_SO1_PNSD { get; set; }
        public decimal? U_SO1_PNCD { get; set; }
        public decimal? U_SO1_DESCUENTO { get; set; }
        public decimal? U_SO1_IMPORTENETO { get; set; }
        public decimal? U_SO1_IMPUESTO { get; set; }
        public int? U_SO1_VENDEDOR { get; set; }
        public decimal? U_SO1_CANTIDADAB { get; set; }
        public string? U_SO1_LISTAMAT { get; set; }
        public string U_SO1_ESPADRE { get; set; } = null!;
        public string U_SO1_TIPOLM { get; set; } = null!;
        public decimal? U_SO1_CANTIDADORIG { get; set; }
        public string? U_SO1_CODIGOALT { get; set; }
        public string? U_SO1_DOCUMENTOBASE { get; set; }
        public short? U_SO1_PARTIDABASE { get; set; }
        public string U_SO1_ANTICIPO { get; set; } = null!;
        public string? U_SO1_PARAMETRO1 { get; set; }
        public string? U_SO1_PARAMETRO2 { get; set; }
        public short? U_SO1_PARAMETRO3 { get; set; }
        public string? U_SO1_COMENTARIOS { get; set; }
        public string? U_SO1_DESCRIPCION { get; set; }
        public string? U_SO1_ALMACEN { get; set; }
        public decimal? U_SO1_CANTIDADVERI { get; set; }
        public string? U_SO1_CODIGOBARRAS { get; set; }
        public string U_SO1_SINONIMO { get; set; } = null!;
        public decimal? U_SO1_CANTIDADSINONI { get; set; }
        public string U_SO1_NOCATALOGADO { get; set; } = null!;
        public string U_SO1_STATUSENTREGA { get; set; } = null!;
        public string U_SO1_TIPOENTREGA { get; set; } = null!;
        public string U_SO1_ESCOMISIONFOPA { get; set; } = null!;
        public string? U_SO1_PROMOCION { get; set; }
        public int? U_SO1_AMBITOPROMO { get; set; }
        public string U_SO1_ESREGALO { get; set; } = null!;
        public decimal? U_SO1_PRECIOBASEPROM { get; set; }
        public string? U_SO1_CODIGOIMPUESTO { get; set; }
        public decimal? U_SO1_IMPUESTOPRCNT { get; set; }
        public string? U_SO1_PARAMETRO4 { get; set; }
        public string? U_SO1_TIPODOCBASE { get; set; }
        public decimal? U_SO1_CANTIDADFAC { get; set; }
        public decimal? U_SO1_IMPUESTOFAC { get; set; }
        public string? U_SO1_RETENCION { get; set; }
        public short? U_SO1_LISTAPRECIO { get; set; }
        public decimal? U_SO1_PESOTEORICO { get; set; }
        public decimal? U_SO1_PESOREAL { get; set; }
        public string U_SO1_TIPOPROMOCION { get; set; } = null!;
        public short? U_SO1_INSTANCIAPROMO { get; set; }
        public string U_SO1_LISTAPRECIOESP { get; set; } = null!;
        public decimal? U_SO1_DCTOPORCENTDX1 { get; set; }
        public decimal? U_SO1_DCTOPORCENTDX2 { get; set; }
        public int? U_SO1_PUNTOS { get; set; }
        public decimal? U_SO1_CIFTOTALPARTID { get; set; }
        public decimal? U_SO1_IMPUESTOCIF { get; set; }
        public decimal? U_SO1_TIPOCAMBMONCIF { get; set; }
        public decimal? U_SO1_PORCENTIMPUCIF { get; set; }
        public string? U_SO1_CERTIFICADOREG { get; set; }
        public string? U_SO1_FORMAPAGOCERTI { get; set; }
        public decimal? U_SO1_COSTO { get; set; }
        public string U_SO1_IMPRESA { get; set; } = null!;
        public int? U_SO1_COMENSAL { get; set; }
        public int? U_SO1_PARTIDASBO { get; set; }
        public string U_SO1_VENTACAMBIO { get; set; } = null!;
        public short? U_SO1_SUBCUENTACOMAN { get; set; }
        public string? U_SO1_TIPOPROMOBASE { get; set; }
        public short? U_SO1_NUMEPROMOBASE { get; set; }
        public string? U_SO1_CODIPROMOBASE { get; set; }
        public string? U_SO1_TIPOPROMOPRIN { get; set; }
        public short? U_SO1_NUMEPROMOPRIN { get; set; }
        public string? U_SO1_CODIPROMOPRIN { get; set; }
        public short? U_SO1_NUMEPROMORMCR { get; set; }
        public string? U_SO1_CODIPROMORMCR { get; set; }
        public string U_SO1_PROMORMREGALO { get; set; } = null!;
        public short? U_SO1_NUMEPROMODICR { get; set; }
        public string? U_SO1_CODIPROMODICR { get; set; }
        public short? U_SO1_NUMEPROMOVDCR { get; set; }
        public string? U_SO1_CODIPROMOVDCR { get; set; }
        public string U_SO1_PRODAUTOMATICA { get; set; } = null!;
        public short? U_SO1_CANTIAPRODUCIR { get; set; }
        public string U_SO1_UNIDADMEDIDAIN { get; set; } = null!;
        public string? U_SO1_CODIUNIMEDINV { get; set; }
        public decimal? U_SO1_CANTUNIMEDINV { get; set; }
        public string U_SO1_ARTIAPLICPROMO { get; set; } = null!;
        public string? U_SO1_PERUTIPOPTAB12 { get; set; }
        public short? U_SO1_PARTIDAPADRE { get; set; }
        public decimal? U_SO1_IMPUESTONETO { get; set; }
        public int? U_SO1_TIEMPO { get; set; }
        public string U_SO1_ESMODIFICADOR { get; set; } = null!;
        public decimal? U_SO1_CANTSINONIMOS { get; set; }
        public string U_SO1_MOSCANTSINO { get; set; } = null!;
        public string? U_SO1_UMISINONIMO { get; set; }
        public decimal? U_SO1_CANTVERISIN { get; set; }
        public string? U_SO1_PERUAFECIMP { get; set; }
        public short? U_SO1_METODOENVIO { get; set; }
        public string? U_SO1_SUCURSAL { get; set; }
    }
}
